...
public:
  ...
  bool begin(uint64_t now_ms, std::string_view local_alias) {
    if (state_ != State::idle || config_.discovery_window_ms == 0 || !valid_alias(local_alias)) return false;
    reset_fields();
  ...
  // Only one candidate is retained; repeated samples for the same rotating alias
  // are harmless, while a second alias cannot grow memory or replace the chosen
  // candidate after the backend request has been committed.
  bool observe_candidate(uint64_t now_ms, std::string_view candidate_alias,
    ...
    if (!candidate_alias_.view().empty() && candidate_alias_.view() != candidate_alias) return false;
    if (!candidate_alias_.set(candidate_alias) || !evidence_id_.set(evidence_id)) return false;
    state_ = State::session_pending;
    return true;
  }

  // message_id from pairing.session_created is the participant-specific nonce.
  // A stale/mismatched candidate alias never advances this FSM.
  bool session_created(uint64_t now_ms, std::string_view candidate_alias,
                       std::string_view session_id, std::string_view confirmation_nonce,
                       uint64_t server_expiry_ms) {
    if (!active_at(now_ms) || state_ != State::session_pending ||
        candidate_alias != candidate_alias_.view() || session_id.empty() || session_id.size() > 128 ||
...
private:
  ...
  FixedText<65> local_alias_{};
  FixedText<65> candidate_alias_{};
