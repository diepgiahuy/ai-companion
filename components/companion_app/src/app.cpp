...
void CompanionApp::tick(uint64_t now_ms) {
  ...
  if (state_ == UiState::listening) {
    ...
    // Feed the exact samples accepted by the speaker into the AEC reference
    // before consuming the next microphone frame. This keeps the portable app
    // ordering deterministic; enclosure/device timing is still a physical gate.
    ...
    // Keep the frontend input drained while idle/ready/processing. WakeNet is
    // acted on only in ready/idle; processing frames are deliberately ignored.
    pump_frontend_monitor(now_ms);
  }
}
...
void CompanionApp::handle_frontend_event(AudioFrontendEvent event, uint64_t now_ms) {
  ...
  if (event == AudioFrontendEvent::speech_started && state_ == UiState::speaking) {
    // Hands-free barge-in intentionally reuses the exact same generation-scoped
    // cancel + canonical begin-turn path as a physical button interruption.
    abort_and_listen(now_ms);
  }
}
