...
2. Use **ESP-IDF `esp_lcd` + `esp_lvgl_port` 2.8.0~1 + LVGL 9** as the one baseline
   UI stack for the first benchmark. Re-verified on **2026-08-14** against the
   official Espressif Component Registry: `2.8.0~1` remains the latest stable
   release. Re-verify immediately before the physical run and record the exact
   resolved artifact/hash rather than silently substituting a newer release.
