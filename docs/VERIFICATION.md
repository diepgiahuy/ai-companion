...
- Go toolchain pinned to **Go 1.26.6** in `backend/go.mod` and the test container.
...
## Full Go 1.26.6 software E2E gate
...
or, in an environment that already has Go 1.26.6 + libopus:

```bash
./scripts/e2e.sh
```

`scripts/e2e.sh` refuses to run the backend suite unless `go env GOVERSION` reports Go 1.26.6. It then executes the full Go suite with the race detector, including WebSocket/Opus, Qwen fake endpoint/tool loop, repository/migration behavior, expense+budget UI/TTS flow, conversation persistence, CRUD, and scheduler alarm ACK/retry tests.

**Release-toolchain boundary:** local runs are useful diagnostics, but only the exact Go 1.26.6 container plus hosted required checks can promote this gate. `make e2e-container` is the release acceptance command.
...
This compatibility gate is designed for sandboxes with no Docker and no outbound module downloads. It runs the C++ host FSM/Opus tests, partition/SRAM checks, then the backend compatibility module. It does not replace the exact Go 1.26.6 production-module gate.
