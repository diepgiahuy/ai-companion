...
- Go toolchain pinned to **Go 1.25.0** in `backend/go.mod` and the test container.
...
## Full Go 1.25 software E2E gate
...
or, in an environment that already has Go 1.25 + libopus:

```bash
./scripts/e2e.sh
```

`scripts/e2e.sh` refuses to run the backend suite unless `go env GOVERSION` reports Go 1.25. It then executes the full Go suite with the race detector, including WebSocket/Opus, Qwen fake endpoint/tool loop, SQLite repositories/migrations, expense+budget UI/TTS flow, conversation persistence, CRUD, and scheduler alarm ACK/retry tests.

**Release-toolchain boundary:** this sandbox has Go 1.23.2 and blocks outbound TCP/DNS, so it cannot fetch the Go 1.25 binary or upstream modules and has no Docker daemon. Functional runtime E2E is now covered by `make e2e-offline`; the exact Go 1.25 + pinned upstream-module release gate remains `make e2e-container` on Docker Desktop/CI.
...
This gate is designed for sandboxes with no Docker and no outbound module downloads. It runs the C++ host FSM/Opus tests, partition/SRAM checks, then `go test -race -count=1 ./...` through `backend/go.offline.mod`. In this sandbox the functional suite executed with Go 1.23.2 because the exact Go 1.25 binary cannot be downloaded; production `backend/go.mod` remains pinned to Go 1.25.0. The compatibility modules use the host `libsqlite3` and `libopus.so.0` and a minimal RFC6455 test transport. Production `go.mod` is untouched.
