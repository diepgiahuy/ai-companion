...
create_voice_mail_relationship(){ local sender="$1" recipient="$2"; (cd "$ROOT/backend" && go run ../host/companion_software_device/create_voice_mail_relationship.go --database-url "$COMPANION_DATABASE_URL" --device-a "$sender" --device-b "$recipient"); }
...
COMPANION_EVIDENCE_CONFIG_SHA256="$(printf '%s\n' 'profile=test' 'auth=database_enrolled' 'protocol=v2' 'voice_mail=ephemeral:paired_relationship:ogg_opus' | sha256sum | awk '{print $1}')"
