...
func ownerAuthFromEnvironment(next http.Handler, claimRepository controlplane.DeviceClaimRepository) http.Handler {
	...
	configured := cfg.AuthorizationURL != "" || cfg.TokenURL != "" || cfg.UserInfoURL != "" || cfg.ClientID != "" || cfg.ClientSecret != "" || cfg.RedirectURL != "" || strings.TrimSpace(os.Getenv("COMPANION_OWNER_OIDC_SCOPES")) != ""
	if !configured { return next }
	service, err := ownerauth.New(cfg)
	if err != nil {
		slog.Error("owner OIDC configuration rejected", "error", err)
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { if strings.HasPrefix(r.URL.Path, ownerPathPrefix) { http.Error(w,"owner authentication unavailable",http.StatusServiceUnavailable); return }; next.ServeHTTP(w,r) })
	}
	...
	if claimRepository != nil {
		...
		if keyErr != nil { slog.Warn("device claim bootstrap disabled", "error", keyErr) } else if claimService, claimErr := onboarding.NewClaimService(claimRepository, service, key); claimErr != nil { slog.Error("initialize device claim service", "error", claimErr) } else { claims = claimService.Handler() }
	}
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/v1/owner/device-claims" { if claims == nil { http.Error(w,"device claim bootstrap unavailable",http.StatusServiceUnavailable); return }; claims.ServeHTTP(w,r); return }
		if strings.HasPrefix(r.URL.Path, ownerPathPrefix) { owner.ServeHTTP(w,r); return }
		next.ServeHTTP(w,r)
	})
}

func ownerScopes(raw string) []string {
	if strings.TrimSpace(raw) == "" { return []string{"openid", "profile"} }
