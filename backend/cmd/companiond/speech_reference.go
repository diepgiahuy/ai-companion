...
const (
	...
	speechProfileGeminiLiveDev      = "gemini-live-dev"
	speechProfileReferenceLocal     = "reference-local"
	speechProfileReferenceStreaming = "reference-streaming"
)

const geminiDelegationInstructions = "You are the voice transport for Companion. For every user audio turn, do not answer or speak first. Call the only available function exactly once and wait for its result. After the function result arrives, speak exactly the string in response.result and nothing else."
...
func configureSpeechComponents(cfg runtimeconfig.Config) (pipeline.Components, error) {
	...
	case speechProfileGeminiLiveDev:
		if cfg.Profile == runtimeconfig.ProfileProduction {
			return pipeline.Components{}, errors.New("gemini-live-dev is forbidden in the production profile")
		}
		apiKey := strings.TrimSpace(os.Getenv("GEMINI_TOKEN"))
		if apiKey == "" {
			return pipeline.Components{}, errors.New("GEMINI_TOKEN is required for gemini-live-dev")
		}
		provider, err := speech.NewGeminiLive(speech.GeminiLiveConfig{
			URL:          strings.TrimSpace(os.Getenv("GEMINI_LIVE_URL")),
			Model:        value("GEMINI_LIVE_MODEL", "gemini-3.1-flash-live-preview"),
			APIKey:       apiKey,
			Voice:        value("GEMINI_LIVE_VOICE", "Kore"),
			Instructions: geminiDelegationInstructions,
			Tools: []speech.NativeRealtimeTool{{
				Name:        "companion_delegate",
				Description: "Mandatory handoff to Companion before any user-visible answer or audio.",
		...
		if err != nil {
			return pipeline.Components{}, fmt.Errorf("configure Gemini Live: %w", err)
		}
		bridge, err := speech.NewGeminiDelegatingBridge(provider)
		if err != nil {
			return pipeline.Components{}, fmt.Errorf("configure Gemini delegation bridge: %w", err)
		}
		components.ASR = bridge
		components.TTS = bridge
		return components, nil
