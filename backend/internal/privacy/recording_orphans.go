...
func defaultRecordingsDir() string {
	if configured := strings.TrimSpace(os.Getenv("COMPANION_RECORDINGS_DIR")); configured != "" {
		return configured
	}
...
func findRecordingOrphans(root string, referenced []string, now time.Time, grace time.Duration) ([]string, error) {
	...
	if grace < 0 {
		return nil, fmt.Errorf("recording orphan grace must not be negative")
	}
	...
	for _, path := range referenced {
		...
		if err != nil {
			return nil, fmt.Errorf("canonicalize referenced recording path: %w", err)
		}
	...
	for _, entry := range entries {
		if entry.IsDir() || entry.Type()&os.ModeSymlink != 0 || !strings.EqualFold(filepath.Ext(entry.Name()), ".wav") {
			continue
		}
		info, err := entry.Info()
		if err != nil {
			return nil, fmt.Errorf("stat recording %q: %w", entry.Name(), err)
		}
		...
		canonical, err := canonicalPath(path)
		if err != nil {
			return nil, fmt.Errorf("canonicalize recording %q: %w", entry.Name(), err)
		}
		if _, ok := references[canonical]; ok {
			continue
		}
