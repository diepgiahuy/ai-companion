...
// OpenAI-compatible Chat Completions transport. It exists for reference models
// such as GLM-4-Flash whose documented API is /chat/completions rather than the
// OpenAI Responses API. Tool execution, sessions, policy and durable state stay
// inside the existing ADK + ToolRegistry path.
...
// pipelineAgent mirrors pipeline.Agent without importing it into the public
// Config surface; newWithModel returns a concrete Runtime satisfying it.
	...
	for _, content := range contents {
		...
		for _, part := range content.Parts {
			...
			case part.FunctionCall != nil:
				...
				calls = append(calls, chatToolCall{
					...
					Function: chatToolCallFunc{Name: part.FunctionCall.Name, Arguments: string(args)},
				})
		...
		for _, response := range responses {
			...
			messages = append(messages, chatMessage{
				Role: "tool", ToolCallID: response.ID, Name: response.Name, Content: string(payload),
			})
		}
	}
	return messages, nil
