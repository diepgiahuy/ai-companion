...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if h.deps.Auth != nil {
		...
		if isMutation {
			session, err = h.deps.Auth.AuthenticateMutation(cookie.Value, r.Header.Get("X-CSRF-Token"))
		} else {
...
func (h *Handler) handleOverview(w http.ResponseWriter, r *http.Request) {
	...
	var goalPtr *domain.SavingsGoal
	if goalSet {
		goalPtr = &goal
	}
	var budgetPtr *int64
	if budgetSet {
		budgetPtr = &monthlyBudget
	}
	savingsProgress := domain.CalculateSavingsProgress(goalPtr, "monthly", startOfMonth, endOfMonth, monthTotal, budgetPtr)
...
func (h *Handler) handleExpenses(w http.ResponseWriter, r *http.Request) {
	...
	items, err := h.deps.Store.ListExpenses(r.Context(), userID, from, to, category, limit)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	total, _ := h.deps.Store.ExpenseTotal(r.Context(), userID, from, to)
...
func (h *Handler) handleJournal(w http.ResponseWriter, r *http.Request) {
	...
	from, to := parseQueryRange(r)
	items, err := h.deps.Store.ListJournal(r.Context(), userID, from, to, parseQueryLimit(r, 50))
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *Handler) handleReminders(w http.ResponseWriter, r *http.Request) {
	...
	deviceID := strings.TrimSpace(r.URL.Query().Get("device_id"))
	limit := parseQueryLimit(r, 50)
	reminders, _ := h.deps.Store.ListReminders(r.Context(), userID, deviceID, "active", limit)
	timers, _ := h.deps.Store.ListTimers(r.Context(), userID, deviceID, "active", limit)
...
func (h *Handler) handleDevice(w http.ResponseWriter, r *http.Request) {
	...
	if deviceID == "" {
		...
		writeJSON(w, emptyOwnerDeviceProjection())
		return
	}
	...
	writeJSON(w, projectOwnerDevice(twin, status, online))
}
...
func (h *Handler) handleGetSavingsGoal(w http.ResponseWriter, r *http.Request) {
	...
	spent, _ := h.deps.Store.ExpenseTotal(r.Context(), userID, start, end)
	budgetLimit, budgetSet, _ := h.deps.Store.BudgetLimit(r.Context(), userID, period)
	var budgetPtr *int64
	if budgetSet {
		budgetPtr = &budgetLimit
	}
	goal, goalSet, err := h.deps.Store.GetSavingsGoal(r.Context(), userID, period)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	progress := domain.CalculateSavingsProgress(goalPtr, period, start, end, spent, budgetPtr)
	writeJSON(w, map[string]any{"period": period, "set": goalSet, "goal": goalPtr, "progress": progress})
}
...
func (h *Handler) handleSetSavingsGoal(w http.ResponseWriter, r *http.Request) {
	...
	if err := mutator.SetSavingsGoalMutation(r.Context(), mutationReq, userID, period, req.TargetVND, strings.TrimSpace(req.Description), time.Now().UTC()); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *Handler) handleCreateReminder(w http.ResponseWriter, r *http.Request) {
	...
	case req.DelayMinutes > 0:
		fireAt = fireAt.Add(time.Duration(req.DelayMinutes) * time.Minute)
	case req.FireAt != "":
		if parsed, err := time.Parse(time.RFC3339, req.FireAt); err == nil {
			fireAt = parsed.UTC()
		}
	default:
...
func (h *Handler) handleGetPrivacy(w http.ResponseWriter, r *http.Request) {
	...
	policy, found, err := h.deps.Store.GetPrivacyPolicy(r.Context(), userID)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *Handler) handleSetPrivacy(w http.ResponseWriter, r *http.Request) {
	...
	req.UserID = userID
	normalized, err := privacy.NormalizePolicy(req)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	normalized.UpdatedAt = time.Now().UTC()
	if err := h.deps.Store.SetPrivacyPolicy(r.Context(), normalized); err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	writeJSON(w, map[string]any{"ok": true, "privacy": normalized})
}
