...
func TestFunASRPostsBoundedWAVToOpenAICompatibleEndpoint(t *testing.T) {
	...
	provider, err := NewFunASR(FunASRConfig{BaseURL: server.URL, Model: "FunAudioLLM/Fun-ASR-MLT-Nano-2512"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if gotModel != "FunAudioLLM/Fun-ASR-MLT-Nano-2512" || gotLanguage != "vi-VN" {
...
func TestFunASRBoundsBufferedTurnAudio(t *testing.T) {
	provider, err := NewFunASR(FunASRConfig{BaseURL: "http://127.0.0.1:1", Model: "m", MaxPCMBytes: 4})
	if err != nil {
		t.Fatal(err)
	}
...
func TestFunASRHTTPCallHonorsCancellation(t *testing.T) {
	...
	defer server.Close()
	provider, err := NewFunASR(FunASRConfig{BaseURL: server.URL + "/v1", Model: "m"})
	if err != nil {
		t.Fatal(err)
	}
	stream, err := provider.StartASR(context.Background(), ASRRequest{Format: AudioFormat{SampleRate: 16000, Channels: 1}}, func(TranscriptEvent) error { return nil })
...
func TestFunASRRejectsOddPCM16AndInvalidConfig(t *testing.T) {
	if _, err := NewFunASR(FunASRConfig{BaseURL: "http://localhost:8000"}); err == nil {
		t.Fatal("missing model should fail")
	}
	provider, err := NewFunASR(FunASRConfig{BaseURL: "http://localhost:8000", Model: "m"})
	if err != nil {
		t.Fatal(err)
	}
	stream, err := provider.StartASR(context.Background(), ASRRequest{Format: AudioFormat{SampleRate: 16000, Channels: 1}}, func(TranscriptEvent) error { return nil })
