...
func TestEdgeTTSUsesUpstreamCLIThenDecodesTo24kPCM(t *testing.T) {
	...
	err = provider.Synthesize(context.Background(), TTSRequest{
		...
		if len(event.PCM) > 0 {
			frames = append(frames, append([]byte(nil), event.PCM...))
		}
		if event.Final {
			final = true
		}
	...
	if len(frames) != 3 {
		t.Fatalf("PCM frames=%d; want 3", len(frames))
	}
	for i, frame := range frames {
		if len(frame) != 2880 {
			t.Fatalf("PCM frame %d length=%d; want 2880", i, len(frame))
		}
	}
	...
	if !bytes.Equal(framed.Bytes()[:len(pcm)], pcm) {
		t.Fatal("framed PCM changed synthesized samples")
	}
	if tail := framed.Bytes()[len(pcm):]; !bytes.Equal(tail, make([]byte, len(tail))) {
		t.Fatalf("final PCM padding contains non-zero bytes: %v", tail)
	}
	if !final {
		t.Fatal("final lifecycle event was not emitted")
	}
...
func TestEdgeTTSPadsPartialFinalFrameWithoutDroppingAudio(t *testing.T) {
	pcm := make([]byte, 2*2880+1728)
	for i := range pcm {
		pcm[i] = byte((i % 250) + 1)
	}
	...
	if !bytes.Equal(framed.Bytes()[:len(pcm)], pcm) {
		t.Fatal("partial final frame lost synthesized PCM")
	}
	padding := framed.Bytes()[len(pcm):]
	if len(padding) != 1152 {
		t.Fatalf("padding bytes=%d; want 1152", len(padding))
	}
	if !bytes.Equal(padding, make([]byte, len(padding))) {
		t.Fatal("partial final frame padding must be silence")
	}
}

func TestEdgeTTSFailsClosedOnSynthesisOrDecodeFailure(t *testing.T) {
	t.Run("synthesis", func(t *testing.T) {
		runner := &fakeEdgeRunner{run: func(_ int, _ edgeRunnerCall) ([]byte, error) {
		...
		if err == nil || !strings.Contains(err.Error(), "synthesize MP3") {
	...
	t.Run("decode", func(t *testing.T) {
		...
		if err == nil || !strings.Contains(err.Error(), "decode EdgeTTS MP3") {
			t.Fatalf("error=%v", err)
		}
	})
}
...
func TestEdgeTTSRejectsWrongFormatAndOddChunkSize(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "24000 Hz mono") {
		t.Fatalf("error=%v", err)
	}
}
