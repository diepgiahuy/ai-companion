...
func TestEdgeTTSContextCancellationAndEmitError(t *testing.T) {
	runner := &fakeEdgeRunner{run: func(index int, _ edgeRunnerCall) ([]byte, error) {
		if index == 0 {
			return []byte("mp3"), nil
		}
		return make([]byte, 5760), nil
	}}
	provider, err := NewEdgeTTS(EdgeTTSConfig{Runner: runner, PCMChunkBytes: 2880})
	if err != nil {
		t.Fatal(err)
	...
	// Early context cancellation
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	err = provider.Synthesize(ctx, TTSRequest{Text: "test", Format: AudioFormat{SampleRate: 24000, Channels: 1}}, func(AudioEvent) error { return nil })
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("expected context.Canceled, got %v", err)
	}

	// Empty text
	err = provider.Synthesize(context.Background(), TTSRequest{Text: "  ", Format: AudioFormat{SampleRate: 24000, Channels: 1}}, func(AudioEvent) error { return nil })
	if err == nil || !strings.Contains(err.Error(), "text is required") {
		t.Fatalf("expected text is required error, got %v", err)
	}

	// Emit callback error
	emitErr := errors.New("stop stream")
	err = provider.Synthesize(context.Background(), TTSRequest{Text: "test", Format: AudioFormat{SampleRate: 24000, Channels: 1}}, func(AudioEvent) error { return emitErr })
