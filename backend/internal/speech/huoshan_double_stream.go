...
func (c HuoshanDoubleStreamTTSConfig) normalized() (HuoshanDoubleStreamTTSConfig, error) {
	...
	if c.URL == "" {
		return c, errors.New("Huoshan double-stream TTS URL is required")
	}
	parsed, err := url.Parse(c.URL)
	if err != nil || parsed.Scheme == "" || parsed.Host == "" {
		return c, fmt.Errorf("invalid Huoshan TTS URL %q", c.URL)
	}
	if parsed.Scheme != "wss" && parsed.Hostname() != "localhost" && parsed.Hostname() != "127.0.0.1" {
		return c, errors.New("Huoshan TTS URL must use wss outside localhost")
	}
	if strings.TrimSpace(c.AppID) == "" || strings.TrimSpace(c.AccessToken) == "" || strings.TrimSpace(c.ResourceID) == "" {
		return c, errors.New("Huoshan TTS app id, access token and resource id are required")
	}
	if strings.TrimSpace(c.Speaker) == "" {
		return c, errors.New("Huoshan TTS speaker is required")
	}
	if c.AudioParams == nil {
		c.AudioParams = map[string]any{"speech_rate": 0, "loudness_rate": 0}
	}
	if c.Additions == nil {
		c.Additions = map[string]any{
			"aigc_metadata": map[string]any{},
			"cache_config":  map[string]any{},
			"post_process":  map[string]any{"pitch": 0},
		}
	}
	return c, nil
}

type HuoshanDoubleStreamTTSProvider struct {
	config HuoshanDoubleStreamTTSConfig
}

func NewHuoshanDoubleStreamTTS(config HuoshanDoubleStreamTTSConfig) (*HuoshanDoubleStreamTTSProvider, error) {
	normalized, err := config.normalized()
	if err != nil {
		return nil, err
	}
	return &HuoshanDoubleStreamTTSProvider{config: normalized}, nil
}
...
func (p *HuoshanDoubleStreamTTSProvider) payload(event int, speaker, text string, sampleRate int) ([]byte, error) {
	audioParams := make(map[string]any, len(p.config.AudioParams)+2)
	for key, value := range p.config.AudioParams {
		audioParams[key] = value
	}
	audioParams["format"] = "pcm"
	audioParams["sample_rate"] = sampleRate
	additions, err := json.Marshal(p.config.Additions)
	if err != nil {
		return nil, fmt.Errorf("marshal Huoshan TTS additions: %w", err)
	}
	...
	if len(p.config.MixSpeaker) > 0 {
		reqParams["mix_speaker"] = p.config.MixSpeaker
	}
	payload := map[string]any{
		"user":      map[string]any{"uid": "companion"},
		"event":     event,
	...
	if err != nil {
		return nil, fmt.Errorf("marshal Huoshan TTS request: %w", err)
	}
	return encoded, nil
}

func huoshanWriteEvent(ctx context.Context, conn *websocket.Conn, event int, sessionID string, payload []byte) error {
	packet, err := buildHuoshanClientPacket(event, sessionID, payload)
	if err != nil {
		return err
	}
...
func buildHuoshanClientPacket(event int, sessionID string, payload []byte) ([]byte, error) {
	if event <= 0 {
		return nil, errors.New("Huoshan event must be positive")
	}
	if len(payload) > int(^uint32(0)>>1) {
		return nil, errors.New("Huoshan payload too large")
	}
	packet := make([]byte, 0, 16+len(sessionID)+len(payload))
	packet = append(packet,
		byte((huoshanProtocolVersion<<4)|huoshanHeaderSize),
		byte((huoshanFullClientRequest<<4)|huoshanFlagWithEvent),
		byte(huoshanJSON<<4),
		0,
	)
...
func parseHuoshanResponse(raw []byte) (huoshanResponse, error) {
	if len(raw) < 4 {
		return huoshanResponse{}, errors.New("Huoshan response shorter than header")
	}
	version := int(raw[0] >> 4)
	headerWords := int(raw[0] & 0x0F)
	if version != huoshanProtocolVersion || headerWords < huoshanHeaderSize {
		return huoshanResponse{}, fmt.Errorf("unsupported Huoshan protocol header version=%d size=%d", version, headerWords)
	}
	offset := headerWords * 4
	if offset > len(raw) {
		return huoshanResponse{}, errors.New("Huoshan response header exceeds frame")
	}
	messageType := int(raw[1] >> 4)
	flags := int(raw[1] & 0x0F)
	response := huoshanResponse{MessageType: messageType}
	if messageType == huoshanErrorInformation {
		code, next, err := readHuoshanInt32(raw, offset)
		if err != nil {
			return response, err
		}
	...
	if messageType != huoshanFullServerResponse && messageType != huoshanAudioOnlyResponse {
		return response, fmt.Errorf("unsupported Huoshan response message type %#x", messageType)
	}
	if flags != huoshanFlagWithEvent {
		return response, fmt.Errorf("Huoshan response missing event flag: %#x", flags)
	}
	event, next, err := readHuoshanInt32(raw, offset)
	if err != nil {
		return response, err
	}
	response.Event = int(event)
	offset = next

	switch response.Event {
	case 50: // ConnectionStarted: connection id only.
		_, _, err := readHuoshanContent(raw, offset)
		return response, err
	case 51: // ConnectionFailed: response metadata only.
		metadata, _, err := readHuoshanContent(raw, offset)
		response.Payload = metadata
		return response, err
	case huoshanEventSessionStarted, huoshanEventSessionFailed, huoshanEventSessionFinished:
		session, next, err := readHuoshanContent(raw, offset)
		if err != nil {
			return response, err
		}
		response.SessionID = string(session)
		metadata, _, err := readHuoshanContent(raw, next)
		if err != nil {
			return response, err
		}
		response.Payload = metadata
...
func appendHuoshanInt32(dst []byte, value int32) []byte {
	var encoded [4]byte
	binary.BigEndian.PutUint32(encoded[:], uint32(value))
	return append(dst, encoded[:]...)
}

func appendHuoshanContent(dst, content []byte) []byte {
	dst = appendHuoshanInt32(dst, int32(len(content)))
	return append(dst, content...)
}

func readHuoshanInt32(raw []byte, offset int) (int32, int, error) {
	if offset < 0 || offset+4 > len(raw) {
		return 0, offset, errors.New("truncated Huoshan int32")
	}
	return int32(binary.BigEndian.Uint32(raw[offset : offset+4])), offset + 4, nil
}

func readHuoshanContent(raw []byte, offset int) ([]byte, int, error) {
	size, next, err := readHuoshanInt32(raw, offset)
	if err != nil {
		return nil, offset, err
	}
	if size < 0 || int64(next)+int64(size) > int64(len(raw)) {
		return nil, offset, errors.New("truncated Huoshan length-prefixed content")
	}
	end := next + int(size)
	return append([]byte(nil), raw[next:end]...), end, nil
}

func huoshanRandomID() (string, error) {
	var raw [16]byte
