...
const xunfeiPCMFrameBytes = 1280 // 40 ms of 16 kHz, 16-bit mono PCM
...
func (c XunfeiStreamASRConfig) normalized() (XunfeiStreamASRConfig, error) {
	if strings.TrimSpace(c.URL) == "" {
		c.URL = "wss://iat-api.xfyun.cn/v2/iat"
	}
	parsed, err := url.Parse(c.URL)
	if err != nil || parsed.Scheme == "" || parsed.Host == "" {
		return c, fmt.Errorf("invalid Xunfei ASR URL %q", c.URL)
	}
	if parsed.Scheme != "wss" && parsed.Hostname() != "127.0.0.1" && parsed.Hostname() != "localhost" {
		return c, errors.New("Xunfei ASR URL must use wss outside localhost")
	}
	if strings.TrimSpace(c.AppID) == "" || strings.TrimSpace(c.APIKey) == "" || strings.TrimSpace(c.APISecret) == "" {
		return c, errors.New("Xunfei ASR app id, API key and API secret are required")
	}
	if c.Language == "" {
		c.Language = "zh_cn"
	}
...
func (p *XunfeiStreamASRProvider) StartASR(ctx context.Context, request ASRRequest, emit func(TranscriptEvent) error) (ASRStream, error) {
	...
	if request.Format.SampleRate != 16000 {
		return nil, fmt.Errorf("Xunfei streaming ASR requires 16000 Hz PCM; got %d", request.Format.SampleRate)
	}

	signedURL, err := xunfeiSignedURL(p.config.URL, p.config.APIKey, p.config.APISecret, p.config.Now())
	if err != nil { return nil, err }
	options := &websocket.DialOptions{}
	if p.config.HTTPClient != nil { options.HTTPClient = p.config.HTTPClient }
	conn, _, err := websocket.Dial(ctx, signedURL, options)
	if err != nil { return nil, fmt.Errorf("dial Xunfei ASR: %w", err) }
	streamCtx, cancel := context.WithCancel(ctx)
	s := &xunfeiASRStream{
		conn: conn, cancel: cancel, config: p.config, emit: emit,
		done: make(chan struct{}), segments: make(map[int]string),
	}
...
func xunfeiSignedURL(rawURL, apiKey, apiSecret string, now time.Time) (string, error) {
	...
	// Go emits UTC; Xunfei requires the HTTP-date GMT spelling.
...
func (s *xunfeiASRStream) finish(err error) {
	s.once.Do(func() {
		s.mu.Lock(); s.err = err; s.mu.Unlock()
		close(s.done)
	})
}
...
func (s *xunfeiASRStream) Push(ctx context.Context, pcm []byte) error {
	...
	if s.closedInput { return errors.New("Xunfei ASR input is already closed") }
	...
	for len(s.pending) >= xunfeiPCMFrameBytes {
		...
		status := 1
		if !s.started { status = 0; s.started = true }
		if err := s.writeFrame(ctx, status, frame); err != nil { return err }
	}
	return nil
}
...
func (s *xunfeiASRStream) CloseInput(ctx context.Context) error {
	...
	s.writeMu.Lock()
	defer s.writeMu.Unlock()
	if s.closedInput { return nil }
	if len(s.pending) > 0 {
		status := 1
		if !s.started { status = 0; s.started = true }
		if err := s.writeFrame(ctx, status, append([]byte(nil), s.pending...)); err != nil { return err }
		s.pending = nil
	}
	if !s.started {
		// Xunfei still needs a status=0 frame to establish business parameters.
		if err := s.writeFrame(ctx, 0, nil); err != nil { return err }
		s.started = true
	}
	if err := s.writeFrame(ctx, 2, nil); err != nil { return err }
	s.closedInput = true
	return nil
}
...
func (s *xunfeiASRStream) writeFrame(ctx context.Context, status int, audio []byte) error {
	...
	if status == 0 {
		business := map[string]any{
			...
			"vinfo": 1,
			"vad_eos": s.config.VADMS,
			"nunum": 1,
			"ptt": 1,
		}
		if s.config.DynamicCorrection { business["dwa"] = "wpgs" }
...
type xunfeiResponse struct {
	...
	Data struct {
		...
		Result struct {
			...
			PGS string `json:"pgs"`
			RG  []int  `json:"rg"`
			WS []struct { CW []struct { W string `json:"w"` } `json:"cw"` } `json:"ws"`
		} `json:"result"`
	} `json:"data"`
}
...
func (s *xunfeiASRStream) readLoop(ctx context.Context) {
	for {
		...
		if err != nil {
			if ctx.Err() != nil { s.finish(ctx.Err()) } else { s.finish(fmt.Errorf("read Xunfei ASR: %w", err)) }
			return
		}
		if kind != websocket.MessageText { continue }
		var response xunfeiResponse
		if err := json.Unmarshal(raw, &response); err != nil { s.finish(fmt.Errorf("decode Xunfei ASR response: %w", err)); return }
		if response.Code != 0 { s.finish(fmt.Errorf("Xunfei ASR code=%d: %s", response.Code, response.Message)); return }
		piece := xunfeiWords(response.Data.Result.WS)
		result := response.Data.Result
		s.mu.Lock()
		if result.PGS == "rpl" && len(result.RG) == 2 {
			for i := result.RG[0]; i <= result.RG[1]; i++ { delete(s.segments, i) }
		}
		if piece != "" { s.segments[result.SN] = piece }
		...
		if text != "" {
			if err := s.emit(TranscriptEvent{Text: text, Final: final, Stable: final}); err != nil { s.finish(err); return }
		} else if final && finalText != "" {
			if err := s.emit(TranscriptEvent{Text: finalText, Final: true, Stable: true}); err != nil { s.finish(err); return }
		}
		if final { s.finish(nil); return }
	}
}

func xunfeiWords(ws []struct { CW []struct { W string `json:"w"` } `json:"cw"` }) string {
	var b strings.Builder
	for _, group := range ws {
		if len(group.CW) > 0 { b.WriteString(group.CW[0].W) }
	}
	return b.String()
}
...
func joinXunfeiSegments(segments map[int]string) string {
	...
	max := 0
	for sn := range segments { if sn > max { max = sn } }
	var b strings.Builder
	for i := 0; i <= max; i++ { b.WriteString(segments[i]) }
	return b.String()
}
...
func (s *xunfeiASRStream) Close() error {
	...
	s.mu.Unlock()
	s.cancel()
	return s.conn.Close(websocket.StatusNormalClosure, "companion Xunfei ASR closed")
}
