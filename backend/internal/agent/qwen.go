...
func WithModelSelector(selector ModelSelector) Option { return func(q *Qwen) { q.modelSelector = selector } }
func WithGenerationConfig(config GenerationConfig) Option { return func(q *Qwen) { q.generation = config } }
func WithPromptBundle(bundle *promptpkg.Bundle) Option { return func(q *Qwen) { q.promptBundle = bundle } }
func WithPersona(persona string) Option { return func(q *Qwen) { q.persona = strings.TrimSpace(persona) } }
func WithUsageMeter(m UsageMeter) Option { return func(q *Qwen) { q.usageMeter = m } }
func WithUsageGuard(g interface { Check(context.Context, string) error }) Option { return func(q *Qwen) { q.usageGuard = g } }
...
func NewQwen(baseURL, apiKey, model, timezone string, turns TurnResultStore, options ...Option) (*Qwen, error) {
	...
	if err != nil { return nil, fmt.Errorf("load timezone: %w", err) }
	q := &Qwen{baseURL: strings.TrimRight(baseURL, "/"), apiKey: apiKey, model: model, location: loc, turns: turns, generation: DefaultGenerationConfig()}
	for _, o := range options { o(q) }
	if err := q.generation.Validate(); err != nil { return nil, fmt.Errorf("generation config: %w", err) }
	q.client = &http.Client{Timeout: q.generation.HTTPTimeout}
	if q.promptBundle == nil {
		q.promptBundle, err = promptpkg.LoadDefault()
		if err != nil { return nil, fmt.Errorf("load default prompt bundle: %w", err) }
	}
	if q.turns == nil { return nil, fmt.Errorf("turn result store is required") }
	if q.toolRegistry == nil { return nil, fmt.Errorf("tool registry is required") }
	return q, nil
}
...
type toolDefinition struct { Type string `json:"type"`; Function map[string]any `json:"function"` }
...
type chatResponse struct {
	...
	Usage struct { PromptTokens int `json:"prompt_tokens"`; CompletionTokens int `json:"completion_tokens"`; TotalTokens int `json:"total_tokens"` } `json:"usage"`
}
...
func (q *Qwen) RespondRich(ctx context.Context, turnID, transcript string) (pipeline.AgentResult, error) {
	...
	if userID == "" { userID = strings.TrimSpace(turn.DeviceID) }
	if q.usageGuard != nil {
		if err := q.usageGuard.Check(ctx, userID); err != nil { return pipeline.AgentResult{}, err }
	}
	...
	if cached, ok, err := q.turns.TurnResult(ctx, turnKey); err != nil { return pipeline.AgentResult{}, err } else if ok { return pipeline.AgentResult{Text: cached}, nil }

	plan := contextengine.Plan{Packs: []string{"expense", "budget", "note", "journal", "schedule", "voice"}}
	if q.planner != nil { plan = q.planner.Plan(ctx, transcript) }
	location := q.location
	if strings.TrimSpace(turn.Timezone) != "" {
		if l, err := time.LoadLocation(turn.Timezone); err == nil { location = l }
	}
	now := time.Now().In(location)
	locale := strings.TrimSpace(turn.Locale)
	if locale == "" { locale = "vi-VN" }
	renderedPrompt, err := q.promptBundle.Render(promptpkg.RenderInput{Locale: locale, CurrentTime: now, Timezone: location.String(), Persona: q.persona, Packs: plan.Packs})
	if err != nil { return pipeline.AgentResult{}, fmt.Errorf("render prompt: %w", err) }
	...
	if q.conversation != nil {
		...
		if history, err := q.conversation.Recent(ctx, scope, 12); err == nil {
			for _, x := range history { messages = append(messages, chatMessage{Role: x.Role, Content: x.Content}) }
		}
	}
	for _, res := range plan.Resources { messages = append(messages, chatMessage{Role: "system", Content: fmt.Sprintf("CONTEXT_RESOURCE %s (%s): %s", res.URI, res.MIMEType, res.Text)}) }
	messages = append(messages, chatMessage{Role: "user", Content: transcript})
	if q.conversation != nil { _ = q.conversation.Append(ctx, turnKey, conversationctx.Scope{UserID: userID, ThreadID: threadID}, "user", transcript) }
	modelTools := q.modelTools(plan.Packs)
	selectedModel := q.model
	if q.modelSelector != nil {
		if m := strings.TrimSpace(q.modelSelector.Select(ctx, transcript)); m != "" { selectedModel = m }
	}
	...
	for round := 0; round < q.generation.MaxToolRounds; round++ {
		...
		if err != nil { return pipeline.AgentResult{}, err }
		messages = append(messages, message)
		if len(message.ToolCalls) == 0 {
			response := strings.TrimSpace(message.Content)
			if response == "" { return pipeline.AgentResult{}, fmt.Errorf("model returned an empty response") }
			if q.conversation != nil { _ = q.conversation.Append(ctx, turnKey, conversationctx.Scope{UserID: userID, ThreadID: threadID}, "assistant", response) }
			if err := q.turns.SaveTurnResult(ctx, turnKey, response); err != nil { return pipeline.AgentResult{}, err }
...
func (q *Qwen) complete(ctx context.Context, messages []chatMessage, tools []toolDefinition, model, promptVersion string) (chatMessage, error) {
	reqBody := chatRequest{Model: model, Messages: messages, Tools: tools, Temperature: q.generation.Temperature, MaxTokens: q.generation.MaxTokens}
	if len(tools) > 0 { reqBody.ToolChoice = "auto"; reqBody.ParallelToolCalls = true }
	...
	if q.apiKey != "" { req.Header.Set("Authorization", "Bearer "+q.apiKey) }
	resp, err := q.client.Do(req)
	if err != nil { return chatMessage{}, err }
	defer resp.Body.Close()
	body, err := io.ReadAll(io.LimitReader(resp.Body, 2<<20))
	if err != nil { return chatMessage{}, err }
	if resp.StatusCode < 200 || resp.StatusCode >= 300 { return chatMessage{}, fmt.Errorf("qwen endpoint returned %s: %s", resp.Status, strings.TrimSpace(string(body))) }
	var decoded chatResponse
	if err := json.Unmarshal(body, &decoded); err != nil { return chatMessage{}, fmt.Errorf("decode qwen response: %w", err) }
	if len(decoded.Choices) == 0 { return chatMessage{}, fmt.Errorf("qwen response has no choices") }
...
func (q *Qwen) modelTools(packs []string) []toolDefinition {
	...
	for _, d := range defs { out = append(out, toolDefinition{Type: "function", Function: map[string]any{"name": d.Name, "description": d.Description, "parameters": d.Parameters}}) }
	return out
}
