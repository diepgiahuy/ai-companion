...
func plain(names ...string) []columnSpec {
	...
	for _, name := range names { out = append(out, c(name, plainValue)) }
	return out
}
func cols(parts ...[]columnSpec) []columnSpec {
	var out []columnSpec
	for _, part := range parts { out = append(out, part...) }
	return out
}
...
var tables = []tableSpec{
	...
	{name: "privacy_policies", columns: cols(plain("user_id"), []columnSpec{c("save_voice_audio", boolValue), c("long_term_memory_enabled", boolValue)}, plain("conversation_retention_days", "voice_memo_retention_days", "memory_retention_days"), []columnSpec{c("updated_at", timeValue)}), orderBy: "user_id"},
...
func ImportSQLite(ctx context.Context, source *sql.DB, target *pgxpool.Pool) (Report, error) {
	if source == nil || target == nil { return Report{}, errors.New("source SQLite and target PostgreSQL are required") }
	if err := checkSQLiteShape(ctx, source); err != nil { return Report{}, err }
	if err := checkPostgresShape(ctx, target); err != nil { return Report{}, err }

	tx, err := target.BeginTx(ctx, pgx.TxOptions{})
	if err != nil { return Report{}, fmt.Errorf("begin PostgreSQL import: %w", err) }
	defer func() { _ = tx.Rollback(context.Background()) }()
	if err := requireFreshTarget(ctx, tx); err != nil { return Report{}, err }
	...
	if _, err := tx.Exec(ctx, `DELETE FROM config_generation`); err != nil { return Report{}, fmt.Errorf("clear config_generation baseline: %w", err) }

	for _, table := range tables {
		if err := importTable(ctx, source, tx, table); err != nil { return Report{}, err }
	}
	for _, table := range outboxTriggerTables {
	...
	for _, table := range tables {
		...
		if _, err := tx.Exec(ctx, query); err != nil { return Report{}, fmt.Errorf("reset %s identity sequence: %w", table.name, err) }
	}
	if err := tx.Commit(ctx); err != nil { return Report{}, fmt.Errorf("commit PostgreSQL import: %w", err) }
...
func importTable(ctx context.Context, source *sql.DB, tx pgx.Tx, table tableSpec) error {
	...
	if err != nil { return fmt.Errorf("read SQLite %s: %w", table.name, err) }
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(dest...); err != nil { return fmt.Errorf("scan SQLite %s: %w", table.name, err) }
		for i, column := range table.columns {
			values[i], err = postgresValue(column.kind, values[i])
			if err != nil { return fmt.Errorf("normalize %s.%s for import: %w", table.name, column.name, err) }
		}
		if _, err := tx.Exec(ctx, insertSQL, values...); err != nil { return fmt.Errorf("insert PostgreSQL %s: %w", table.name, err) }
	}
	if err := rows.Err(); err != nil { return fmt.Errorf("iterate SQLite %s: %w", table.name, err) }
	return nil
}
...
func requireFreshTarget(ctx context.Context, tx pgx.Tx) error {
	for _, table := range tables {
		var count int64
		if err := tx.QueryRow(ctx, "SELECT count(*) FROM "+quoteIdent(table.name)).Scan(&count); err != nil { return fmt.Errorf("count PostgreSQL %s: %w", table.name, err) }
		if table.name == "config_generation" {
			if count != 1 { return fmt.Errorf("target config_generation must contain only Atlas baseline row, got %d rows", count) }
			var id, version int64
			if err := tx.QueryRow(ctx, `SELECT id,version FROM config_generation`).Scan(&id, &version); err != nil { return err }
			if id != 1 || version != 1 { return fmt.Errorf("target config_generation baseline is not 1:1") }
			continue
		}
		if count != 0 { return fmt.Errorf("target PostgreSQL is not fresh: %s has %d rows", table.name, count) }
	}
	return nil
}
...
func checkSQLiteShape(ctx context.Context, db *sql.DB) error {
	for _, table := range tables {
		...
		if err != nil { return fmt.Errorf("inspect SQLite %s: %w", table.name, err) }
		...
		for rows.Next() {
			...
			if err := rows.Scan(&cid, &name, &typ, &notNull, &defaultValue, &pk); err != nil { rows.Close(); return err }
			seen[name] = true
		}
		rows.Close()
		for _, column := range table.columns { if !seen[column.name] { return fmt.Errorf("SQLite schema missing %s.%s", table.name, column.name) } }
	}
	return nil
}
...
func checkPostgresShape(ctx context.Context, pool *pgxpool.Pool) error {
	for _, table := range tables {
		...
		if err != nil { return fmt.Errorf("inspect PostgreSQL %s: %w", table.name, err) }
		seen := map[string]bool{}
		for rows.Next() { var name string; if err := rows.Scan(&name); err != nil { rows.Close(); return err }; seen[name] = true }
		rows.Close()
		for _, column := range table.columns { if !seen[column.name] { return fmt.Errorf("PostgreSQL schema missing %s.%s", table.name, column.name) } }
	}
	return nil
}
...
func digestSQLiteTable(ctx context.Context, db *sql.DB, table tableSpec) (TableDigest, error) {
	...
	if err != nil { return TableDigest{}, fmt.Errorf("digest SQLite %s: %w", table.name, err) }
	defer rows.Close()
	h := sha256.New(); var count int64
	for rows.Next() {
		values := make([]any, len(table.columns)); dest := make([]any, len(values)); for i := range values { dest[i] = &values[i] }
		if err := rows.Scan(dest...); err != nil { return TableDigest{}, err }
		canonical, err := canonicalRow(table, values); if err != nil { return TableDigest{}, err }
		h.Write(canonical); h.Write([]byte{'\n'}); count++
	}
	if err := rows.Err(); err != nil { return TableDigest{}, err }
...
func digestPostgresTable(ctx context.Context, pool *pgxpool.Pool, table tableSpec) (TableDigest, error) {
	...
	if err != nil { return TableDigest{}, fmt.Errorf("digest PostgreSQL %s: %w", table.name, err) }
	defer rows.Close()
	h := sha256.New(); var count int64
	for rows.Next() {
		values, err := rows.Values(); if err != nil { return TableDigest{}, err }
		canonical, err := canonicalRow(table, values); if err != nil { return TableDigest{}, err }
		h.Write(canonical); h.Write([]byte{'\n'}); count++
	}
	if err := rows.Err(); err != nil { return TableDigest{}, err }
...
func canonicalRow(table tableSpec, values []any) ([]byte, error) {
	if len(values) != len(table.columns) { return nil, fmt.Errorf("%s row has %d values, want %d", table.name, len(values), len(table.columns)) }
	out := make([]any, len(values))
	for i, column := range table.columns {
		normalized, err := canonicalValue(column.kind, values[i]); if err != nil { return nil, fmt.Errorf("canonicalize %s.%s: %w", table.name, column.name, err) }
		out[i] = normalized
	}
...
func canonicalValue(kind valueKind, value any) (any, error) {
	...
	switch kind {
	case boolValue:
		switch v := value.(type) { case bool: return v, nil; case int64: return v != 0, nil; case int32: return v != 0, nil; case int: return v != 0, nil; case []byte: return parseBool(string(v)); case string: return parseBool(v) }
		return nil, fmt.Errorf("unsupported boolean %T", value)
	case timeValue, nullableTimeValue:
		if t, ok := value.(time.Time); ok { return t.UTC().Format(time.RFC3339Nano), nil }
		raw := strings.TrimSpace(stringValue(value))
		if raw == "" && kind == nullableTimeValue { return nil, nil }
		t, err := parseTime(raw); if err != nil { return nil, err }
		return t.UTC().Format(time.RFC3339Nano), nil
	case jsonValue:
		var decoded any
		switch v := value.(type) { case []byte: if err := json.Unmarshal(v, &decoded); err != nil { return nil, err }; case string: if err := json.Unmarshal([]byte(v), &decoded); err != nil { return nil, err }; default: decoded = v }
		return decoded, nil
	default:
		if b, ok := value.([]byte); ok { return string(b), nil }
		return value, nil
	}
}
...
func postgresValue(kind valueKind, value any) (any, error) {
	...
	case timeValue, nullableTimeValue:
		normalized, err := canonicalValue(kind, value); if err != nil || normalized == nil { return normalized, err }
		return time.Parse(time.RFC3339Nano, normalized.(string))
	case jsonValue:
		normalized, err := canonicalValue(kind, value); if err != nil { return nil, err }
		encoded, err := json.Marshal(normalized); if err != nil { return nil, err }
...
func parseBool(raw string) (bool, error) {
	switch strings.ToLower(strings.TrimSpace(raw)) { case "1", "true": return true, nil; case "0", "false": return false, nil; default: return false, fmt.Errorf("invalid boolean %q", raw) }
}

func parseTime(raw string) (time.Time, error) {
	for _, layout := range []string{time.RFC3339Nano, time.RFC3339, "2006-01-02 15:04:05.999999999Z07:00", "2006-01-02 15:04:05"} {
		if parsed, err := time.Parse(layout, raw); err == nil { return parsed, nil }
	}
	return time.Time{}, fmt.Errorf("invalid timestamp %q", raw)
}

func stringValue(value any) string {
	switch v := value.(type) { case string: return v; case []byte: return string(v); default: return fmt.Sprint(v) }
}

func columnList(table tableSpec) string {
	columns := make([]string, len(table.columns)); for i, column := range table.columns { columns[i] = quoteIdent(column.name) }; return strings.Join(columns, ",")
}
