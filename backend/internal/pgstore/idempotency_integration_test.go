...
func semanticJSONEqual(t *testing.T, left, right string) bool {
	t.Helper()
	var a any
	if err := json.Unmarshal([]byte(left), &a); err != nil {
		t.Fatalf("decode left JSON %q: %v", left, err)
	}
	var b any
	if err := json.Unmarshal([]byte(right), &b); err != nil {
		t.Fatalf("decode right JSON %q: %v", right, err)
	}
...
func TestPostgresIdempotencyReplayConflictAndActorIsolation(t *testing.T) {
	...
	first, err := RunIdempotent(ctx, pool, request, mutate)
	if err != nil {
		t.Fatal(err)
	}
	second, err := RunIdempotent(ctx, pool, request, func(pgx.Tx) (any, error) {
	...
	if first.Replayed || !second.Replayed || !semanticJSONEqual(t, first.JSON, second.JSON) || calls.Load() != 1 {
		t.Fatalf("first=%+v second=%+v calls=%d", first, second, calls.Load())
	}

	conflictReq := request
	conflictReq.RequestHash = hashB
	if _, err := RunIdempotent(ctx, pool, conflictReq, func(pgx.Tx) (any, error) {
		t.Fatal("conflict executed mutation")
		return nil, nil
	}); !idempotency.IsConflict(err) {
		t.Fatalf("conflict error=%v", err)
	}

	actor2 := request
	actor2.Actor = prefix + "-actor-2"
	if _, err := RunIdempotent(ctx, pool, actor2, func(tx pgx.Tx) (any, error) {
		return map[string]any{"actor": 2}, nil
	}); err != nil {
...
func TestPostgresIdempotencyConcurrentFirstAttemptMutatesOnce(t *testing.T) {
	...
	if mutations.Load() != 1 {
		t.Fatalf("mutation count=%d want=1", mutations.Load())
	}
	var baseline string
	replayed := 0
	for out := range results {
		...
		if !semanticJSONEqual(t, out.JSON, baseline) {
			t.Fatalf("outcome mismatch: %q != %q", out.JSON, baseline)
		}
		if out.Replayed {
			replayed++
		}
	...
	var rows int
	if err := pool.QueryRow(ctx, `SELECT count(*) FROM notes WHERE user_id=$1`, prefix).Scan(&rows); err != nil {
		t.Fatal(err)
	}
	if rows != 1 {
		t.Fatalf("note rows=%d want=1", rows)
	}
}
