...
func (s *Store) UpsertMemory(ctx context.Context, item memory.Item) (memory.Item, error) {
	if item.UserID == "" || item.Key == "" || item.Value == "" { return item, fmt.Errorf("user, key and value required") }
...
func (s *Store) ApplyRetention(ctx context.Context,now time.Time)(privacy.RetentionReport,error){rows,err:=s.pool.Query(ctx,`SELECT user_id,conversation_retention_days,voice_memo_retention_days,memory_retention_days FROM privacy_policies WHERE conversation_retention_days>0 OR voice_memo_retention_days>0 OR memory_retention_days>0`);if err!=nil{return privacy.RetentionReport{},err};defer rows.Close();type policyRow struct{user string;conversation,voice,memory int};var policies []policyRow;for rows.Next(){var p policyRow;if err:=rows.Scan(&p.user,&p.conversation,&p.voice,&p.memory);err!=nil{return privacy.RetentionReport{},err};policies=append(policies,p)};if err:=rows.Err();err!=nil{return privacy.RetentionReport{},err};var report privacy.RetentionReport;for _,p:=range policies{if p.conversation>0{tag,err:=s.pool.Exec(ctx,`DELETE FROM conversation_messages WHERE user_id=$1 AND created_at<$2`,p.user,now.Add(-time.Duration(p.conversation)*24*time.Hour).UTC());if err!=nil{return report,err};report.ConversationRows+=tag.RowsAffected()};if p.memory>0{tag,err:=s.pool.Exec(ctx,`DELETE FROM memories WHERE user_id=$1 AND created_at<$2`,p.user,now.Add(-time.Duration(p.memory)*24*time.Hour).UTC());if err!=nil{return report,err};report.MemoryRows+=tag.RowsAffected()};if p.voice>0{cutoff:=now.Add(-time.Duration(p.voice)*24*time.Hour).UTC();voiceRows,err:=s.pool.Query(ctx,`SELECT path FROM voice_memos WHERE user_id=$1 AND created_at<$2`,p.user,cutoff);if err!=nil{return report,err};var paths []string;for voiceRows.Next(){var path string;if err:=voiceRows.Scan(&path);err!=nil{voiceRows.Close();return report,err};paths=append(paths,path)};if err:=voiceRows.Err();err!=nil{voiceRows.Close();return report,err};voiceRows.Close();tag,err:=s.pool.Exec(ctx,`DELETE FROM voice_memos WHERE user_id=$1 AND created_at<$2`,p.user,cutoff);if err!=nil{return report,err};report.VoiceMemoRows+=tag.RowsAffected();report.OrphanPaths=append(report.OrphanPaths,paths...)}};return report,nil}
