...
(ROOT / "backend/internal/store/idempotency_core_test.go").write_text(r'''package store
...
func TestIdempotentMutationReplayConflictActorAndRetry(t *testing.T) {
    ...
    if err != nil || first.Replayed || calls != 1 { t.Fatalf("first commit outcome=%+v calls=%d err=%v", first, calls, err) }
    ...
    if _, err := s.runIdempotentMutation(ctx, failure, func(*sql.Tx) (any, error) { return nil, boom }); !errors.Is(err, boom) { t.Fatalf("expected precommit failure, got %v", err) }
    if _, err := s.runIdempotentMutation(ctx, failure, func(tx *sql.Tx) (any, error) {
        ...
        return map[string]any{"saved": true}, err
    }); err != nil { t.Fatalf("precommit failure was cached: %v", err) }
}
...
func TestIdempotentMutationSurvivesRestart(t *testing.T) {
    ...
    if err != nil || !replay.Replayed || replay.JSON != first.JSON || called { t.Fatalf("restart replay first=%+v replay=%+v called=%v err=%v", first, replay, called, err) }
}
...
''', encoding="utf-8")
print("issue27 core ledger patch applied")
