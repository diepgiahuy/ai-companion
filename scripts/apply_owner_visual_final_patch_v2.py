#!/usr/bin/env python3
"""One-shot remediation v2 for #281. Removed before merge."""
...
replace_once(
    html,
    '.content{padding:26px 14px 96px}.head h1{font-size:21px;line-height:25px}.quick-grid,.split,.metrics,.requested-applied,.advanced-grid,.form-grid{grid-template-columns:1fr}',
...
replace_once(
    html,
    'function pageHead(title,_lead){return `<div class="head"><div><h1>${esc(title)}</h1><div class="lead">Current product surface</div></div><span class="live">LIVE</span></div>`}',
    'function pageHead(title,lead){return `<div class="head"><div><h1>${esc(title)}</h1><div class="lead">${esc(lead)}</div></div><span class="live">LIVE</span></div>`}',
)
...
print("owner visual final patch v2 applied")
