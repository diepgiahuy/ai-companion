...
The script uses the Hugging Face Dataset Viewer rows API so CI downloads only the
selected samples instead of the multi-hundred-MB parquet shards.  It records the
resolved dataset revision and SHA-256 of the normalized 16-kHz mono PCM used by
providers.  The mixed case is a deterministic concatenation of one Vietnamese
and one English human recording and is explicitly labelled composite_recorded.
"""
...
def main() -> int:
    ...
    if len(revision) < 12:
        raise RuntimeError("could not resolve immutable google/fleurs revision")
    ...
    print(f"prepared {len(cases)} cases from {DATASET}@{revision}")
    for case in cases:
        print(f"{case['id']}: {case['pcm_sha256']} {case['duration_ms']:.1f}ms {case['source_kind']}")
    return 0
